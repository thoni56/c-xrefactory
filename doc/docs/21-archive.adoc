...
The workaround highlighted the underlying tension: **navigation and refactoring used different sources of truth**. The proper fix — eliminating `callXref()` from the refactoring path entirely — landed with server-side refactoring (link:15-roadmap.adoc[Chapter 15: Roadmap]).
...
`updateSessionReferencesForFile`, described in link:11-algorithms.adoc[Chapter 11].
...
Variant A cleared; variant B open and unprobed. The candidate fix — strip a CU's
transitive include closure before reparsing it — addresses variant B's mechanism but was
not pursued, since the manifestation it targets is unconfirmed.
