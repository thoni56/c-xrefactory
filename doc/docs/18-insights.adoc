...
1. `removeReferenceableItemsForFile()` - removes old references FROM the stale file
2. `parseToCreateReferences()` - re-parses, adds fresh references FROM that file to in-memory table
3. Clear menu refs - necessary so step 4 doesn't reject as duplicates
4. `scanReferencesToCreateMenu()` - reload ALL refs from disk database (cross-file refs)
5. `extendBrowserMenuWithReferences()` - merge in-memory refs (fresh positions from current file)

The key insight: after step 2, the in-memory table only has references FROM the current file. Cross-file references (from other files TO symbols here) must come from the disk database via step 4.
