...
NOTE: This section describes the refactoring architecture. The `callXref()` pattern described here is still used by refactoring operations. However, the server mode description below is partially outdated — the server now reparses stale preloaded files at entry and parses discovered CUs at startup (link:/workspace/decisions#20[ADR-0020]). See link:15-roadmap.adoc[Chapter 15: Roadmap] for the path toward eliminating `callXref()` from refactoring as well.
