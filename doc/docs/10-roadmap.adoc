...
|Server-side restructuring (link:/workspace/1/decisions/C-xrefactory#20[ADR-0020]): the server entry point reparses stale files before dispatching any operation. Pass 1 reparses stale CUs, Pass 2 walks the reverse-include graph (via `TypeCppInclude` references) to find and reparse CUs that include stale headers. At startup, the server loads the `.cx` snapshot (`loadFileNumbersFromStore`), discovers CUs by globbing the project directory, and parses only stale CUs (mtime changed since snapshot). The Emacs client only preloads modified buffers, so unmodified files don't trigger unnecessary reparsing. Operations can assume the in-memory table is current.

|**Lightweight file structure scanning (done)**
|CU discovery by globbing the project directory, include structure extraction from `#include` lines, populating the same `TypeCppInclude` references that full parsing creates. Operational on the auto-detect path. See link:/workspace/1/decisions/C-xrefactory#22[ADR-0022].
...
|**References refresh module (function exists, wiring deferred)**
|A shared module with a unified interface: "ensure these files have fresh references", supporting different _file selection strategies_ — the caller decides which files need freshness, the module handles the reparsing.

Current state: the `ensureFreshReferences` function exists with a unit test, but has no production callers. The refactoring path still depends on entry refresh having already run before the operation begins. Wiring the module into the refactoring entry path is deferred.
...
|**CreateMode — batch prime & config validation (proposed)**
|A standalone `c-xref -create [path]` batch mode that starts exactly like `-server` (auto-discovery, snapshot load), then eagerly parses the whole project, reports parse errors on stderr (exiting non-zero on failure), writes the snapshot, and exits. No command-line options — the discovered `.c-xrefrc` is authoritative. Create and update are unified: the snapshot is a disposable cache, always loaded and reconciled, so there is no separate `-update`. Built as a thin reuse of the server parse path, it gives `-create` a server-path home — the move that unblocks removing XrefMode. See link:/workspace/1/decisions/C-xrefactory#24[ADR-0024].
...
Both navigation and refactoring already use the same in-memory `ReferenceableItemTable` (see link:20-insights.adoc[Chapter 20: Unified In-Memory Table Discovery] for details). The infrastructure for memory-as-truth is largely already in place:
...
1. **Lightweight scanning replaces the mandatory `-create`** (done) — discover CUs and include structure without full parsing. Populate the same `TypeCppInclude` references. (An *optional* `-create` returns as a deliberate batch prime — see CreateMode, link:/workspace/1/decisions/C-xrefactory#24[ADR-0024].)
...
An optional **CreateMode** (`c-xref -create`, link:/workspace/1/decisions/C-xrefactory#24[ADR-0024]) performs this whole-project parse deliberately and up front — for priming a large project or validating the project config — without reintroducing a _required_ create step.

==== Related Decisions and Details

* **link:/workspace/1/decisions/C-xrefactory#5[ADR-0005]**: Automatically find config files — enables project discovery by upward search
* **link:/workspace/1/decisions/C-xrefactory#13[ADR-0013]**: No archaic extern patterns — limits symbol visibility to include-based, making include closure complete
* **link:/workspace/1/decisions/C-xrefactory#14[ADR-0014]**: Adopt on-demand parsing architecture — formalizes this direction
* **link:/workspace/1/decisions/C-xrefactory#20[ADR-0020]**: Separate sync from dispatch — implemented, entry-point reparse with include-graph walking
* **link:/workspace/1/decisions/C-xrefactory#21[ADR-0021]**: Single-project server policy — foundation for index-based sessions
* **link:/workspace/1/decisions/C-xrefactory#22[ADR-0022]**: Lightweight file structure scanning — replaces `-create` and `callXref()`
...
See link:#move-function-refactoring[Chapter 11: Move Function Between Files] for implementation details and phase breakdown.
