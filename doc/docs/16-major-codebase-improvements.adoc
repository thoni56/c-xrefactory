...
This chapter documents larger architectural changes and improvements that would significantly enhance code quality, maintainability, and performance but require careful planning and implementation.

These are distinct from the user-facing refactorings documented in "Refactoring Recipes" - those are operations users apply to their code, while these are improvements to c-xrefactory's own codebase.

=== LexemStream API Improvements

**Status**: PARTIALLY COMPLETE (November 2025)
...
✓ **Basic API functions** (`src/lexemstream.c`, `src/lexemstream.h`):
- `lexemStreamHasMore()` - Check if stream has more lexems
- `copyNextLexemFromStreamToStream()` - Copy one lexem between streams
- `skipExtraLexemInformationFor()` - Skip lexem metadata

✓ **Partially refactored functions**:
- `copyRemainingLexems()` - Now uses `LexemStream` API (yylex.c:1794)
- `replaceMacroArguments()` - Refactored to use `LexemStream` (commit 73239531)
- `expandMacroArgument()` - Takes `LexemStream *argumentInput` (yylex.c:1708)
- `resolveMacroArgumentAsLeftOperand()` - Partially uses API (commit 06e7e17e)

**What remains to be done:**

✗ **Extended LexemStream structure** - No ownership fields yet:
- Missing: `allocatedSize`, `ownsBuffer` fields
- Structure still lightweight (5 fields only)

✗ **Full API not created** - Missing functions:
- Creation: `lexInputCreateOwned()`, `lexInputCreateView()`
- Capacity: `lexInputEnsureCapacity()`, `lexInputReallocIfNeeded()`
- Writing: `lexInputPutLexemCode()`, `lexInputAppendBytes()`
- Cleanup: `lexInputFree()`

✗ **Major functions still using old style**:
- `collate()` - Still has 5 pointer parameters (yylex.c:2022)
- Several helper functions: `collate_id_id()`, `collate_const_const()`, etc.

**Progress estimate**: ~40% complete (basic API exists, some refactoring done, but major work remains)
...
✓ **Basic LSP communication**: Server startup, initialization, shutdown
✓ **textDocument/didOpen**: Parse and index files as they're opened
✓ **textDocument/definition**: Go-to-definition for global symbols
...
✗ **Local variables**: Go-to-definition not supported

- Cause: Local variables not indexed in `ReferenceableItemTable` during background/'xref' parsing
- Legacy Emacs mode works because it reparses with cursor position, making locals available in transient tables
- LSP background indexing has no cursor, so locals aren't tracked
- Fix requires: Store locals in reference table OR implement scope-based lookup

✗ **Multi-file symbols**: Symbols in other files not accessible

- Only the currently opened file is parsed and indexed
- Cross-file references require include graph and on-demand parsing (see "Parsing Subsystem" section)

==== Code Locations

**LSP Infrastructure**:

* `src/lsp.c` - LSP server main loop and message handling
* `src/lsp_handler.c` - Request/notification handlers  
* `src/lsp_adapter.c` - Translate LSP requests to c-xrefactory queries
* `src/lsp_sender.c` - JSON-RPC response formatting
* `src/json_utils.c` - JSON construction helpers

**Key functions**:

* `initializeParsingSubsystem()` - Initialize options and tables for LSP mode
* `parseToCreateReferences()` - Parse file and populate reference database
* `findDefinition()` - Look up symbol at position, return definition location

==== Architecture Notes

**Options initialization**: LSP mode now calls `deepCopyOptionsFromTo(&presetOptions, &options)` to get baseline configuration including file suffixes, avoiding the expensive project discovery and compiler interrogation that XRef mode performs.

**Language detection**: Derives language from filename extension via `getLanguageFor()`, consistent with XRef mode. LSP protocol's `languageId` field is currently ignored.

**Reference database**: In-memory `ReferenceableItemTable` populated by parsing. Currently cleared on each parse; future work will make it persistent across files.

==== Next Steps

To achieve feature parity with legacy Emacs mode:

1. **Local variable support**: Either index locals in reference table, or implement function-scope lookup
2. **Multi-file support**: Implement include graph and on-demand parsing (foundational work in "Parsing Subsystem" section)
3. **Additional LSP features**: References, hover, completion, rename

==== Related Work

* **Parsing Subsystem Refactoring** - Provides foundation for multi-file support
* **ADR-0014** (On-Demand Parsing) - Architecture for efficient cross-file lookups
* **Unified Symbol Database** - Will enable persistent indexing across files
