...
NOTE: There is still ongoing work is to try to identify modules/components, which are
not always directly mapped to source files, but based on higher level responsibilites.


==== CxrefProgram

image::embed:CxrefProgram[]

`cxrefProgram` is the core container. It does all the work when it comes
...
and the 'cxrefProgram` container is a back-and-forth communication using a
...
NOTE: There is a proof of concept implementation of a rudimentary LSP adapter which
would make it possible to use `c-xrefactory` from a wide range of editors and IDEs, at
least for many operations.
