...
**Current**: Include relationships are already tracked as `TypeCppInclude` references in the `referenceableItemTable` (link name `"%%%i"`). Each reference encodes: `ReferenceableItem.includeFileNumber` = included file, `Reference.position.file` = including file. These are created by full parsing and persisted in the `.cx` database. The entry-point reparse (ADR 20) already walks this graph transitively to find CUs including stale headers.

**Remaining gap**: These references are only populated by full parsing or loaded from the disk db. Lightweight file structure scanning (link:/workspace/decisions#22[ADR-0022]) would populate them from `#include` line scanning, without full parsing — making the include graph available even without a `.cx` file or prior full parse.
