...
For planned user-facing features, see link:11-planned-features.adoc[Chapter 11: Planned Features].

=== Incremental `cxfile.c` Cleanup

==== Background

The `cxfile` module was designed when the `.cx` files on disk were the reference database. Operations like "show all references to symbol X" or "find unused symbols" were table-driven scans over the disk format — hash the symbol name, stream the right partition, apply a per-operation filter. This was efficient for the batch cross-referencer that `c-xrefactory` evolved from, where the workflow was `c-xref -create` → `c-xref -update` → query the on-disk database.

Under memory-as-truth (link:10-roadmap.adoc[Chapter 10]), all operations now read from the in-memory `referenceableItemTable`; the `.cx` files serve only as a startup snapshot.

==== Status

The major cleanup is complete. The operation-specific disk scanners have been removed, replaced by in-memory walks:

* `scanForGlobalUnused` — unused-symbol detection
* `scanForSearch` — project-wide search
* `scanReferencesToCreateMenu` — browsing menu construction
* `scanForMacroUsage` — macro expansion file lookup

The disk-load helper that fed several of them has also been removed:

* `ensureReferencesAreLoadedFor` — include-refs disk load; obsolete once include refs come from the lightweight scan and full parsing

A disk-read tripwire in `scanCxFile` asserts that no `.cx` read happens after the initial snapshot load. See "Memory As Truth" in link:06-principles.adoc[Principles].

The current public interface in `cxfile.h` is:

* `loadSnapshotFromStore`, `loadFileNumbersFromStore` — startup loaders
* `saveReferencesToStore` — snapshot writer
* `cxFileHashNumberForSymbol` — partition-hash helper
* `searchSymbolCheckReference` — per-reference search matcher

==== Remaining

Two small interface tightenings — not major improvements, but worth listing as known loose ends:

* **Make `cxFileHashNumberForSymbol` static.** It's a partitioning implementation detail; the single external use (menu sorting in cxref.c) can be rerouted.
* **Make `searchSymbolCheckReference` static.** The per-reference search matcher belongs in `search.c`. Moving it removes the last reason the storage module needs to know about search string matching.

==== References

* `src/cxfile.h`, `src/cxfile.c` — current implementation
* `src/search.c` — natural home for search matching logic
...
Session and Project are tangled in time as well as in the struct. `mainTaskEntryInitialisations()` (`startup.c:827`) runs project discovery at process start, using `cwd` when the command line names no file, and reads that project's options before any request has arrived. So "set once at server startup" already includes project-scoped work.
...
Compounding the problem, `processOptions` is not a pure parser. It opens the output file, turns each `-D` into a macro definition, and triggers file-processing initialization. Reading the config is not pure either. It picks the pass sections by looking at the global `currentPass`, and it raises the global `maxPasses` when it sees a `-passN` marker. Those side effects are why the global-mutation pattern is load-bearing today: parsing and its consequences are entangled.
...
**2. Extract side effects from option parsing.** Parsing becomes pure; side effects remain where they belong but with a clear trigger point. This is the load-bearing step — without it, typed requests still have to round-trip through global state.

The comment in `loadProjectSettings` that ends in "!!! berk" describes the knot. Request options must be read before the config, the cached options after it, and the cache must not hold request options. With no side effects to replay, the order stops mattering.

A likely order, cheapest first:

[loweralpha]
. *Pass count as data.* The pass sections are already read into `ProjectConfig.optionSets`. The parse loops can take the number of passes from there, instead of setting `maxPasses` to 1 and relying on a config read to raise it.
. *Read the config without a pass.* Per-pass options already arrive through `applyOptionSet()`, so the config read can be base-only. The loaded settings are then the same in every pass, and a change of pass no longer forces a full reload. This should also stop a cold start from interrogating the compiler twice (`tests/test_cold_start_interrogates_compiler_once`).
. *Defines when a parse starts.* `-D` is saved in `definitionStrings` and also turned into a macro immediately. Applying the saved strings when a parse starts takes the config's defines out of the memory checkpoint.
. *The rest.* Output-file opening and file-processing initialization move into the dispatcher.

The order comes from reading the code and has not been tried. The first attempt at the compiler test also changed three move function tests, for a reason not yet understood.
...
Server operations use the prefix `olcx` (likely "on-line cross-references"), a legacy abbreviation that conveys no meaning to anyone reading the code or protocol today. Option names like `-olcxpush`, `-olcxtagsearch`, `-getproject are opaque compared to what they could be: `-push`, `-search`, `-get-project-name`.
...
=== Elisp Compiled and Deleted on Every Build

==== Problem Statement

Building `c-xref` runs `$(MAKE) -C ../editors/emacs` from the `c-xref` recipe, and
that Makefile's `all` target does this:

[source,make]
----
all: c-xrefprotocol.el compile
	-rm *.elc # Remove the compiled elisp since we want to load source in emacs-devel (utils)
...
So every build deletes any `.elc` files, byte-compiles all the elisp, then deletes
the result again. `compile` is a phony target with no prerequisites, so this happens
whether or not any `.el` file changed. It costs roughly two seconds per build here,
which is most of the time an otherwise up-to-date `make c-xref` takes, and it makes
`make test` (which now depends on `c-xref`) pay the same on every run.

The comment says why the output is thrown away: `utils/emacs-devel` loads the
sources, not the compiled files. So the compilation is not for using the result —
it is a *check* that the elisp still compiles, and byte-compiler warnings are the
real product.

==== Scope

Decide what the step is for and make it say so:

* If it is a syntax and warning check, it belongs with the tests rather than in
  every link of `c-xref`, and it should fail the build when the compiler reports
  an error (today both commands are prefixed with `-`, so failures are ignored).
* If the compiled files are wanted for people who load them, they should not be
  deleted, and `emacs-devel` should be the one choosing sources.
* Either way it should be incremental: `%.elc: %.el`, so an unchanged tree costs
  nothing.

The protocol generation (`c-xrefprotocol.el` from `protocol.th`) is already a
proper rule with prerequisites and should stay as it is.
