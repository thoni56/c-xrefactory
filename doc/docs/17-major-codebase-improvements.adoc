...
Under memory-as-truth (link:15-roadmap.adoc[Chapter 15]), all operations now read from the in-memory `referenceableItemTable`; the `.cx` files serve only as a startup snapshot.
