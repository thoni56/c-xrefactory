...
`-xrefrc` is on its way out — link:/workspace/1/decisions/C-xrefactory#5[ADR-0005] called
for removing it once discovery existed, and discovery exists — so this legacy path is
expected to go with it rather than to stay as a second way in.
