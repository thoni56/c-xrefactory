...
runs when no project root was auto-detected, i.e. no `.c-xrefrc` was found above the
request file. See
<<unified-flow>> below.
...
(`initializeFileProcessing`) runs when no project root was detected. The flow below
describes the auto-detect path.
...
something like (from the read_xrefs test case):
    ...
    -encoding=european -olcxpush -urldirect  "-preload" "<file>" "-olmark=0" "-olcursor=6" "<file>" -p "<project>"
    end-of-options
...
The configuration file is a `.c-xrefrc` in the root of the project, found by searching
upward from the file being worked on. The search stops at the user's home directory, so a
`~/.c-xrefrc` from the old user-central model is not read.
...
- The project's configuration file, `.c-xrefrc`
...
(`detectedProjectRoot`). If none is found, the file is not in any project. The `-getproject`
