...
**File Modification Tracking** (`filetable.h`, `caching.c`):
...
**Include Dependency Tracking** (`yylex.c:474`, `caching.c:323-334`):
...
void pushInclude(FILE *file, EditorBuffer *buffer, char *name, char *prepend) {
    ...
    cacheInclude(currentFile.characterBuffer.fileNumber);  // Record dependency
}

// Cascading validation: if ANY included file changed, parent needs rescan
static bool cachedIncludedFilePass(int index) {
    for (int i = cache.points[index - 1].includeStackTop; i < includeTop; i++) {
        if (!checkFileModifiedTime(cache.includeStack[i]))
            return false;  // ANY included file changed = need rescan
    }
    return true;
}
----

**Automatic Include Closure** (`xref.c:81-111`):
