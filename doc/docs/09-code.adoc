...
NOTE: The current architecture is being restructured toward a unified server flow where
lightweight file structure scanning replaces the legacy `-create`/`callXref()` pattern.
See <<target-unified-flow>> below and ADR 22 for the target design. Annotations marked
[TARGET] indicate planned changes.
...
[TARGET] `processFileArguments()` will be replaced by `scanProjectForFilesAndIncludes()`,
which discovers CUs by globbing the project directory rather than relying on command-line
file lists. This also builds the include graph transitively by text-scanning `#include`
lines and resolving paths via `options.includeDirs`. See <<target-unified-flow>>.
...
Since ADR 20, `callServer()` performs a staleness reparse phase before dispatching each
request. This is not shown in the legacy flow diagram below but is now the first action
in every request after initialization.
...
server() [server.c:237] - Infinite request loop
  ...
  └─> FOR EACH REQUEST:
       ...
       └─> callServer(baseArgs, requestArgs, &firstPass) [server.c:214]
            ...
            │
            ├─> IF projectContextInitialized:              ← ADR 20
            │    └─> reparseStalePreloadedFiles()
            ...
            │
            ├─> IF NOT projectContextInitialized:
            │    ├─ OP_ACTIVE_PROJECT: initializeProjectContext()
            │    │   → processFileArguments() → loadFileNumbersFromStore()
            │    │   → parseDiscoveredCompilationUnits()
            │    ├─ requiresProcessingInputFile: processFile() (legacy)
            │    └─ other: error
...
The two-pass reparse (ADR 20) is critical for correctness: Pass 1 reparses stale CUs,
which updates their `TypeCppInclude` references. Pass 2 then queries those freshened
references to find which CUs include stale headers. A user cannot create a new include
edge without editing the includer, so the includer is always stale when the edge is new.
...
[TARGET] `initializeProjectContext` will be eliminated — it duplicates phases 1-4 of
`initializeFileProcessing`. With the unified flow, all initialization goes through
`loadProjectSettings()` once at startup. The multi-project fast-path optimization
(checkpoint restore when same project) becomes dead code with single-project servers
(ADR 21).
...
[[target-unified-flow]]
==== Target: Unified Server Flow (ADR 22)

The current architecture has two initialization paths (`initializeProjectContext` for
`OP_ACTIVE_PROJECT`, `initializeFileProcessing` for legacy per-file) and relies on the
disk database as the source of truth for project structure. The target design unifies
these into a single path where the in-memory database is authoritative.
...
server() loop:
  ...
  │     │    TypeCppInclude refs
  │     │  → Catches new files disk db doesn't know about
  │     │
  │     ├─ Reconcile: mark missing CUs as deleted
  │     │  → markMissingFilesAsDeleted(): CUs only (glob-discoverable)
  │     │  → Header disappearance detected naturally on reparse
  │     │
  │     └─ projectContextInitialized = true
  │
  ├─ 2. Update what changed (same two-pass as ADR 20)
  │     ├─ Pass 1: reparse stale CUs
  │     └─ Pass 2: reparse CUs that include stale headers
  │     Also: if config changed → re-run loadProjectSettings + scan
  │
  ├─ 3. Execute operation
  │
  └─ 4. Repeat from 2
