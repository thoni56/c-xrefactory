...
NOTE: Historically, `callXref()` was also called by the refactory to re-index files
after refactoring operations. Refactorings now run as server operations (`OP_REFACTORY`),
using the server's own entry refresh mechanism instead.
...
==== File Discovery and the File Table
...
**Project directory glob** (`scanProjectForFilesAndIncludes()` → `listFilesInDirectory()`). At startup, after project context initialization, the server walks the project root recursively, filtering by known source suffixes (`.c`, `.y`, etc.) and honoring `-prune` paths (applied in `listFilesInDirectory` against the project-root-relative `-prune` entries). This discovers all compilation units in the project directory without requiring a prior `-create` invocation. (The legacy `processFileArguments()` path over `options.inputFiles` remains only as a fallback when no project root is auto-detected.)
...
* **Adapter** (`lsp_adapter.c`) — bridges LSP concepts to `c-xrefactory` internals. `findDefinition()` converts LSP URI and position to an internal `Position`, queries the `referenceableItemTable` directly, and returns an LSP location JSON object
...
* The LSP adapter queries `referenceableItemTable` directly
...
Refactoring operations run as server operations (`OP_REFACTORY`) within the long-lived
editor server process. The editor sends the refactoring request (e.g. `-rfct-rename`,
`-rfct-extract-function`) as a server command, and the server responds with edit
instructions via the protocol.

The refactoring code saves options into `refactoringOptions` to preserve the refactoring
intent while the server's entry refresh and parsing machinery runs with its own options.

==== Operation Flow

A typical refactoring (e.g. rename):

1. Editor sends `-rfct-rename -renameto=NEW_NAME -olcursor=POSITION FILE`
2. Server parses the file to identify the symbol at the cursor
...
|Reaped after each request unless held (markers, preload). The reaping pass is `closeAllEditorBuffersIfClosable`, governed by `bufferIsCloseable()`. link:/workspace/1/decisions/C-xrefactory#23[ADR-0023] proposes a more uniformly persistent model.

|**Server mode, disk cache**
|Subject to the same closeable-check; the disk-cache role typically allows reaping, but a buffer held by markers (mid-refactoring) is preserved.

|**Xref mode, disk cache**
|Preserved across file processing under the same predicate. A header like `commons.h` included by many CUs is read once and reused.
...
That single-file read is what the partitioning was for, and nothing reads the database
that way any more: it is a startup snapshot, loaded whole and never consulted again. The
partitioning and the `-refnum` that sizes it are therefore being removed, together with
`-refs` — see link:/workspace/1/decisions/C-xrefactory#28[ADR-0028]. What is described
above is the layout as it still exists.
...
The preload mechanism writes the buffer content to a temporary file and passes both the original filename and the temp file path. The server creates an `EditorBuffer` with the content and the temp file's modification time. Preloaded buffers persist across requests as long as the client keeps sending them. On each preload, the server compares the new temp file's mtime against the existing buffer's; if advanced, the buffer is reloaded with the new content, and the per-request entry refresh reparses it before the operation runs. When the client stops sending a preload (because the user saved or closed the file), the server detects this and prunes the now-stale buffer (ghost-buffer cleanup).