...
`textDocument/codeAction` can offer a refactoring, but has no standard way
to ask the user for input while it runs.
...
How much of this can still reach LSP editors, and what each editor would need
in its own extension, is covered in
<<10-roadmap.adoc#lsp-refactoring-reach,Roadmap: Reaching refactorings>>.
