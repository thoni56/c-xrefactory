...
The default mode is "xref". The command line option `-server` selects server mode.
Branching is done in the final lines in `main()`.

Refactoring operations run within the server process (via `OP_REFACTORY`), not as a
separate mode. The refactoring code saves and restores options via `refactoringOptions`
to preserve its intent while the server's parsing machinery operates.
