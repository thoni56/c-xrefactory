...
|Reaped after each request unless held (markers, preload). The reaping pass is `closeAllEditorBuffersIfClosable`, governed by `bufferIsCloseable()`. link:/workspace/decisions#23[ADR-0023] proposes a more uniformly persistent model.
