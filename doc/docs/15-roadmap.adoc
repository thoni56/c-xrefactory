...
----
                  ...
                  (convergence point)
                   /              \
                  v                v
    References refresh      Index-based sessions
    module (replaces       (keys + positions,
    callXref)                not copies)
           |                      |
           v                      v
    ┌─────────────────────────────────────┐
...
3. **References refresh module** — extract reparse logic into a shared module callable from both entry refresh (broad staleness) and operations (symbol-targeted). Replaces `callXref()` in refactoring with precise, on-demand reparsing.
