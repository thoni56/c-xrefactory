...
----
                  ...
                  /         |         \
                 v          v          v
    Refactoring as     Index-based    Search reads
    server operation   sessions       from in-memory
    (strangler fig)    (keys +        table
            |          positions,        |
            v          not copies)       |
    References refresh      |            |
    module (done for        |            |
...
The header-filtered sibling parsing optimization (see Optimization section) becomes another file selection strategy within this module. See link:18-insights.adoc[Chapter 18: RefactoryMode Internally Calls XrefMode] for the current architecture this replaces.

|**Refactoring as server operation (strangler fig)**
|Refactoring moves from a separate process (`RefactoryMode`) into the running server as `OP_REFACTORY`. Migrated one refactoring at a time — old path stays alive until all are proven. Rename is operational end-to-end, validated on real project (tests: `test_rename_via_server`, `test_rename_via_server_multi_file`, Emacs debug-mode toggle). Mark-stale-after-rename ensures safety check sees fresh references in sibling files. Remaining refactorings to be migrated one at a time. Once all refactorings work in-server, `parseBufferUsingServer` (the bridge — see link:17-major-codebase-improvements.adoc[17.3]) dissolves: symbol resolution and reference lookup become direct queries against `referenceableItemTable`, eliminating the round-trip through `callServer`. Removing the old path also removes `callXref` and the `cxMemory` wipe that currently destroys the entire reference table during refactoring.
...
*Clean exit* (done): `-exit` handler saves snapshot. `kill-emacs-hook` sends `-exit`.

*Project change* (done): `c-xref-kill-xref-process` sends `-exit` before killing the process.

*All buffers saved* (not yet): when no editor buffers are modified, the in-memory state is consistent with disk — a clean moment to snapshot. This would be the primary trigger during normal workflow.

Dirty kills (crash, `kill -9`, terminal closed) lose in-memory state — the next cold start reparses from scratch. Acceptable since clean exit and project change cover normal workflow.
|===
...
3. **Refactoring moves into server (strangler fig)** — refactoring becomes a server operation (`OP_REFACTORY`), reusing the server's in-memory references instead of rebuilding from scratch. Rename is migrated: `ensureReferencesAreUpdated`/`callXref` skipped in server path, mark-stale ensures safety check correctness across files. Remaining refactorings follow the same pattern. Final step: replace `parseBufferUsingServer` with direct table queries (see link:17-major-codebase-improvements.adoc[17.3]), then remove `RefactoryMode`.
4. **`.cx` becomes startup snapshot only** — never consulted during operation. The in-memory table is authoritative. Search reads from memory.
