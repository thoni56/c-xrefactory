...
----
                  ...
                  (convergence point)
                   /              \
                  v                v
    Refactoring as        Index-based sessions
    server operation      (keys + positions,
    (strangler fig)         not copies)
           |                      |
           v                      |
    References refresh            |
    module (replaces              |
...
|**References refresh module (replaces callXref)**
...
Entry refresh (sync phase) calls it with broad staleness-based selection (same as current three passes). Refactoring operations call it with symbol-targeted selection: resolve the symbol, look up which files contain its references in `referenceableItemTable`, ensure those files are fresh, then proceed. This replaces the coarse `UPDATE_DEFAULT/FAST/FULL` levels with precise, data-driven selection — a local variable naturally has references in one file, a header-defined function naturally has references in many. No scope categorization needed.
...
|Refactoring moves from a separate process (`RefactoryMode`) into the running server as `OP_REFACTORY`. Migrated one refactoring at a time — old path stays alive until all are proven. Rename is operational (test: `test_rename_via_server`, Emacs debug-mode toggle). Removing the old path also removes `callXref`, `parseBufferUsingServer`, and the `cxMemory` wipe that currently destroys the entire reference table during refactoring.
...
3. **Refactoring moves into server (strangler fig)** — refactoring becomes a server operation (`OP_REFACTORY`), reusing the server's in-memory references instead of rebuilding from scratch. Rename is migrated first, remaining refactorings follow one at a time. Inside each migrated refactoring, `ensureReferencesAreUpdated`/`callXref` is replaced by `ensureFreshReferences` — symbol-directed, precise reparsing that preserves the reference table instead of destroying it. Once all refactorings are migrated, `RefactoryMode` and `parseBufferUsingServer` are removed.
