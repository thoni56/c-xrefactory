...
|**Client sends all modified buffers**
|Emacs client sends `-preload` for all modified buffers, not just the current file. Without this, the server is blind to modifications in non-current files. See link:18-insights.adoc[Chapter 18: Navigation Architecture and the Preloading Limitation] for why this matters.
