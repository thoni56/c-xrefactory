...
==== 4. Unified Symbol Database Architecture

*Reference*: link:../adr/0014-adopt-on-demand-parsing-architecture.md[ADR-0014: Adopt On-Demand Parsing Architecture]

*Priority*: HIGH (Enables true on-demand LSP)

*Rationale*: Transition from batch-mode (mandatory `-create`) to on-demand parsing where operations work immediately. The `.cx` files become a transparent cache rather than a prerequisite.
