...
|**Refactoring as server operation (strangler fig — operational)**
|All refactorings run as server operations via `OP_REFACTORY` in `serverPerformRefactoring()`: rename, extract (function/macro/variable), parameter manipulation (add/del/move), move function, organize includes, rename include/module. Validated on real projects via Emacs. `markModifiedEditorBuffersAsStale()` ensures entry refresh detects refactoring-modified buffers. Emacs client has a "Server-side Refactoring" toggle in the Misc menu (`c-xref-server-refactoring`) for battle-testing before removing the legacy path. The legacy `-refactory` process path remains alive but unused when the toggle is on. Final cleanup: remove `RefactoryMode`, `callXref`, `parseBufferUsingServer` bridge (see link:17-major-codebase-improvements.adoc[17.3]), and the `cxMemory` wipe.
