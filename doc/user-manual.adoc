...
C-xrefactory auto-detects projects based on the file you're editing. The first time you use any c-xref function, you'll be prompted to create a project. The active project is shown in the modeline after each action.
...
You can browse multiple symbols simultaneously. Each push adds to a stack:

1. While browsing `variable`, you see it passed to `function`
2. Push `function` to see its definition
3. Next/Previous now navigate `function`'s references
4. Pop to return to browsing `variable`

Use *Forward* to re-push a symbol you popped by mistake.
