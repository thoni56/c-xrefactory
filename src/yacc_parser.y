...
primary_expr
    : IDENTIFIER            {
        ...
        if (symbol != NULL && symbol->type == TypeDefault) {
            ...
            assert(options.mode);
            $$.data.reference = addCxReference(symbol, &$1.data->position, UsageUsed, NO_FILE_NUMBER, NO_FILE_NUMBER);
        } else {
...
user_defined_type
    : TYPE_NAME                                             {
        ...
        if (nestingLevel() == 0)
            ...
            usage = UsageUsed;
        addCxReference($1.data->symbol,&$1.data->position,usage,NO_FILE_NUMBER,NO_FILE_NUMBER);
    }
    ;
