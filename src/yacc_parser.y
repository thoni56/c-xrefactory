...
*/
    ...
    |   '%' UNION {
        ...
        '{' struct_declaration_list '}' {
            specializeStructOrUnionDef($<typeModifier>3->typeSymbol, $5.data);
            l_yaccUnion = $<typeModifier>3->typeSymbol;
        }
...
token_seq
    :   IDENTIFIER              {
            addYaccSymbolReference($1.data, UsageDeclared);
        }
...
rules
    :    Start_block IDENTIFIER ':'                          {
            addYaccSymbolReference($2.data, UsageDefined);
            addRuleLocalVariable($2.data, 0);
        }
        rule_bodies End_block ';'
    |   rules Start_block IDENTIFIER ':'                    {
            addYaccSymbolReference($3.data, UsageDefined);
            addRuleLocalVariable($3.data, 0);
        }
        rule_bodies End_block ';'
    | COMPLETE_YACC_LEXEM_NAME             { assert(0); /* token never used */ }
    | rules COMPLETE_YACC_LEXEM_NAME       { assert(0); /* token never used */ }
    ;
...
rule_body
    : lexem                   {
            if ($1.data != NULL) {
                addYaccSymbolReference($1.data, UsageUsed);
                addRuleLocalVariable($1.data, 1);
            }
            $$.data = 2;
        }
    |   compound_statement      {
    ...
    |   rule_body lexem         {
            if ($2.data != NULL) {
                addYaccSymbolReference($2.data, UsageUsed);
                addRuleLocalVariable($2.data, $1.data);
            }
            $$.data = $1.data + 1;
        }
...
external_definition
    ...
    | function_definition_head {
        ...
        if (parsedInfo.function != NULL
            ...
            && positionIsBetween(parsingConfig.positionOfSelectedReference, $1.begin, $3.end)) {
