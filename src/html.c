...
static void htmlGenLogo(void) {
    fprintf(communicationChannel,"<p><font size=-1><center>\n");
    fprintf(communicationChannel, "Html form generated by ");
    fprintf(communicationChannel, "<A HREF=\"https://github.com/thoni56/c-xrefactory/\" target=\"_top\">");
    fprintf(communicationChannel,"<B>C-xrefactory</B></A>");
    fprintf(communicationChannel," version %s ",C_XREF_VERSION_NUMBER);
    fprintf(communicationChannel," on %s\n", ctime(&s_fileProcessStartTime));
    fprintf(communicationChannel,"</center></font><p>\n");
}
...
static void htmlGenHead(int fn) {
    ...
    fprintf(communicationChannel,"<HTML><HEAD><TITLE>%s</TITLE>\n",fname);
    if (options.htmlNoUnderline) {
        fprintf(communicationChannel,"<STYLE TYPE=\"text/css\">\n");
        fprintf(communicationChannel,"<!-- A:link{text-decoration: none}\n");
        fprintf(communicationChannel,"A:visited{text-decoration: none}\n");
        fprintf(communicationChannel,"A:active{text-decoration: none} -->\n");
        fprintf(communicationChannel,"</STYLE>\n");
    }
    fprintf(communicationChannel,"</HEAD>\n");
    fprintf(communicationChannel,"<body topmargin=\"0\" leftmargin=\"0\"");
    fprintf(communicationChannel,"bgcolor=\"#FFFFFF\"\n");
    fprintf(communicationChannel,"link=\"%s\" vlink=\"%s\" alink=\"red\">\n",
            options.htmlLinkColor, options.htmlLinkColor);
    fprintf(communicationChannel,"<center>");
    if (GENERATE_FRAMES()) {
        fprintf(communicationChannel,"<font size=-1>");
        fprintf(communicationChannel,"<A HREF=\"XFRM%c%s.frm.html%s\" ",
                HTML_DIRECTORY_SEPARATOR, simpleFileName(fname),options.htmlLinkSuffix);
        //      fprintf(ccOut,"target=\"_top\"");
        fprintf(communicationChannel,">");
        fprintf(communicationChannel,"Create Xref-Html Frames</A>");
        for(i=0; i<10; i++) fprintf(communicationChannel,"&nbsp; ");
        fprintf(communicationChannel,"<A HREF=\"%s.html%s\" target=\"_top\">",
                htmlCutLastSuffixStatic(simpleFileName(fname)),
                options.htmlLinkSuffix);
        fprintf(communicationChannel,"Remove All Frames</A>");
        fprintf(communicationChannel,"</font>");
        fprintf(communicationChannel,"<hr>");
    }
    fprintf(communicationChannel,"file:<B>");
    sn = 0;
    ...
    while (sn > 0) {
        ...
        if (cutFlag && ss0<cutfn) {
            fprintf(communicationChannel,"%s/", ss0);
        } else {
            fprintf(communicationChannel,"<A HREF=\".%c",HTML_DIRECTORY_SEPARATOR);
            for(i=1; i<sn; i++) fprintf(communicationChannel,"..%c",HTML_DIRECTORY_SEPARATOR);
            fprintf(communicationChannel,"\">%s</A>/", ss0);
        }
        *ss = ch;
        ss0 = ss+1;
        sn--;
    ...
    fprintf(communicationChannel,"%s",ss0);
    fprintf(communicationChannel,"</B>");
    fprintf(communicationChannel," &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(%s) ",
            ctime(&fileTable.tab[fn]->lastModified));
    fprintf(communicationChannel,"</center>");
    fprintf(communicationChannel,"<hr>\n");
    fprintf(communicationChannel,"<pre>\n");
}

static void htmlGenTail(void) {
    int i;
    for(i=0;i<40;i++) fprintf(communicationChannel,"\n");
    fprintf(communicationChannel,"</pre>\n");
    fprintf(communicationChannel,"<hr>\n");
    htmlGenLogo();
    fprintf(communicationChannel,"</BODY></HTML>\n");
}
...
static void htmlGenRefListItemHead(FILE *ff, char *ln, char *symName,
    ...
    if (p!=NULL && p->b.symType==TypeCppInclude) {
        ...
        if (p->b.symType==TypeStruct && LANGUAGE(LANG_JAVA)) {
            ...
            fprintf(ff,"%s&nbsp;", typeNamesTable[p->b.symType]);
        }
        ld = sn;
...
static char * getDefaultCxFileStatic(void) {
...
void htmlGetDefinitionReferences(void) {
    char *tt;
    UNUSED tt;
...
static int htmlRefListOrdering(Reference *r1, Reference *r2) {
    /*
...
static char *htmlStSymbolCode(SymbolReferenceItem *r, int usage) {
    ...
    if (r->b.category==CategoryLocal) {
        ...
        if (r->vApplClass == noFileIndex) {
            sprintf(ss,"%s.%s", typeNamesTable[r->b.symType], r->name);
        } else {
            ...
            if (r->b.symType == TypeDefault) {
                ...
                sprintf(ss,"%s.%s.%s", typeNamesTable[r->b.symType], tt, r->name);
            }
        }
...
static void fillHtmlRefList(S_htmlRefList *rref, SymbolReferenceItem *symbolRefItem, Reference *reference,
...
static void htmlGenGlobRefList(FILE *ff, char *fname,
    ...
    if (count != 0) {
        ...
        fprintf(ff,"<tr><td><pre>\n");
        LIST_MERGE_SORT(Reference, p->refs, olcxListLessFunction);
        /*&
          LIST_LEN(llen, Reference, p->refs);
...
static void htmlJavaDocPosProcess(  FILE **fff,
    ...
    if (rr->reference->usage.base == UsageJavaDocFullEntry
        ...
        if (dr!=NULL) {
            ...
            if (usage == UsageJavaDocFullEntry) fprintf(communicationChannel," /**");
            fprintf(communicationChannel,"\n  * @see %s[Source]%s\n",prf,suf);
            fprintf(communicationChannel,"  * @see %s[Xrefs]%s\n",prf1,suf1);
            if (usage == UsageJavaDocFullEntry) fprintf(communicationChannel,"  */\n");
        }
    }
    rr = rr->next;
...
static void htmlPosProcess( FILE **fff,
    ...
    if (cri->b.symType == TypeFunSep) {
        if (options.htmlFunSeparate) {
            fprintf(communicationChannel, "</pre><font size= -1 color=\"red\"><hr><center>");
            fprintf(communicationChannel,"%s:%d",
                    fileTable.tab[cp->file]->name, cp->line);
            fprintf(communicationChannel, "</center><hr></font><pre>");
        }
        goto fini;
    } else if (cri->b.symType == TypeComment) {
        if (rr->reference->usage.base == UsageDefined) {
            fprintf(communicationChannel,"<font color=\"green\">");
        } else {
            fprintf(communicationChannel,"</font>");
        }
        goto fini;
    ...
    if (htmlPositionLess(&rr->reference->p, cp)) {
        fprintf(communicationChannel,"%s%s",prf0,prf);
        fprintf(communicationChannel,"%s%s",suf,suf0);
    } else {
        if (    cri->b.symType==TypeCppIfElse
        ...
        if (rr->reference->usage.base==UsageDefined) {
            if ((rr->symbolRefItem->b.category==CategoryLocal && options.htmllocalx)
                ...
                fprintf(communicationChannel,"%s%s",prf0,prf1);
                HtmlPassSourceIdent(cp,ch,ff);
                fprintf(communicationChannel,"%s%s",suf1,suf0);
            } else {
                fprintf(communicationChannel,"%s",prf0);
                HtmlPassSourceIdent(cp,ch,ff);
                fprintf(communicationChannel,"%s",suf0);
            }
            ...
            fprintf(communicationChannel,"%s%s",suf1,prf);
            HtmlPassSourceIdent(cp,ch,ff);
            fprintf(communicationChannel,"%s%s",suf,suf0);
        } else {
            fprintf(communicationChannel,"%s%s",prf0,prf);
...
static void htmlPassRefsThroughSourceFile(S_htmlRefList **rrr, int ifile,
    ...
    ch = '\n';
    if (genFlag==GEN_HTML) htmlPutCharLF(communicationChannel, ch, &cp);
    else fputc(ch,communicationChannel);
    if (cofile!=NULL) {
...
static void htmlGenerateFile(int fnum) {
    ...
    recursivelyCreateFileDirIfNotExists(ffn);
    communicationChannel = openFile(ffn,"w");
