...
* TODO Feature: Handle UTF correctly
  ...
  We should remove the current handling and replace it with support for only UTF-8,
  probably by using character positions as the interface and adjusting indices into the
