...
***** DOING Replace all FILL_symbol() calls
***** TODO Move Symbol struct to Symbol-module
***** DONE Create a Symbol module with newSymbolxxx()
...
* TODO Add an -exit option so that client can take down server cleanly
