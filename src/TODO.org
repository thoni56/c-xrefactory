...
Current Mikado:
* DOING Menu option to upgrade 'c-xrefactory' - it should, in principle, just be "git pull, kill c-xref, make prod, reload c-xref.el, c-xrefactory.el"
...
* DOING A switch that creates a list of commands given to c-xref so that they can be replayed exactly later
...
Well, no. I tried to get started with doing refactorings inside
clangd, but getting started was to cumbersome...
