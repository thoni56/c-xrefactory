...
* DOING Introduce adapter for stat() in fileio
...
* TODO Turn the out parameter "position" of XXXTabAdd to return value instead, so it can be ignored without unused variable
* TODO Handle void as no parameters in formal argument lists, e.g. when refactoring them
* TODO BUG: an unterminated "assert(" (with the c-xref def of it) breaks PUSH
Instead it results in 
error in process filter: if: SYNCHRO PROBLEM: 
12:06:15 FATAL yylex.c:1809: '((char *)(buf)) + (bufsize+1000)*sizeof(char) == ppmMemory + ppmMemoryIndex' is not true
