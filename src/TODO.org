...
* TODO More Yacc syntactic sugar constructs that might be supported:
named references: "$name" or "$<tag>name"
* TODO Client menu Project->Delete still works with the $HOME/.c-xrefrc
* TODO BUG? Forward declarations
It seems that all references to `expandMacroCall` in yylex.c is found, more importantly,
the actual definition is not.
* TODO "Search Definition" vs "Search Symbol"
* TODO BUG: Misses the references in c_parser.y for TypeBool
* TODO Still losing some cx(?) memory after pro-longed periods of usage
* TODO Testcase for modified included file
One of the key features is to be able to understand when files needs to be
rescanned. I'm not sure how that works, but we need tests for it.
* TODO Only do full updates, or what is the drawback of the non-full updates?
...
** DONE Fill ParsedInfo begin/end for requested function
** DONE Fixup the preliminary version of the actual move
** DONE Move any declaration in source.h file to target.h
...
* TODO Add yacc parser construct for field in yystype ('$<field>$' and '$<field>3')
  ...
  have it.
* TODO Make Emacs client verify arguments better (e.g. Add Parameter - position should be number)
...
* TODO ROADMAP
** DONE Project local .c-xrefrc with convention-based database (v1.9)
** TODO Make filenames relative in .c-xrefrc so it can be checked into version control
...
** DONE FEATURE: Organize includes - first own .h, then system <>, then local, each group alphabetically
...
** DONE FEATURE: Move function to other module
...
* TODO Separate the options into sections and move them to separate structures
  The options structure is used to save or possibly create new sets, when
...
* TODO Maybe: Select extra defines as necessary depending on compiler discovered as a sideeffect of discoverStandardIncludes()
...
* DONE Bug: Extract function handles variables declared inside the region used after it
  The declaration is no longer moved into the extracted function; the variable
  is returned (or passed back via out-parameter) so post-region code keeps
  working. Fix detects implicit-decl `extern int <name>()` shadows left in the
  reference table — the forensic trail of the synthetic `{}` region-scope
  popping region-locals before post-region parsing — and splices them into the
  program graph as outside-block nodes pointing at the region-local's
  referenceableItem. Existing dataflow classification handles the rest.
  Test: tests/test_extract_local_declared_in_region_used_after/.
  (Was previously two TODOs: test_c_extract_function_with_out_variables and
  the BUGS entry "declaration moved to extracted function".)
* DONE FEATURE: Basic LSP-mode support
** DONE 'didOpen'
** DONE Handle protocol errors etc. using longjump
** DONE LSP communication structure
** DONE LSP 'initialize', 'exit', 'not found'
* DONE Project local .c-xrefrc with convention-based database location
   New projects use project-local .c-xrefrc with database at .c-xref/db (v1.9).
   Legacy ~/.c-xrefrc projects continue to work via fallback.
** DONE Reference database stored in project tree at .c-xref/db
   Convention-based path applied automatically when using project-local config.
** DONE Make "get project" lookup the directory tree for closest .c-xrefrc
   Upward search from source file, stops at HOME to avoid finding ~/.c-xrefrc.
** TODO Make all filenames relative in the options file
   Would allow .c-xrefrc to be checked into version control.
** DONE Simplified project creation wizard in Emacs
   Detects git root, creates minimal .c-xrefrc, offers to generate database.
** TODO Migrate legacy config to project-local
   Feature to move a section from ~/.c-xrefrc to project-local .c-xrefrc.
** DONE Function to lookup the closest .c-xrefrc and potentially use its directory path as project name
* DONE Store Xrefs in .c-xref/db parallel to the .c-xrefrc used (v1.9)
  Convention-based database path for project-local configs. Legacy -refs option still works for
  backward compatibility with existing ~/.c-xrefrc configurations.
* DONE FEATURE: LSP Find Definition
** DONE Initiate server so it can handle OP_PUSH
** DONE Create an answer from gathered information
** DONE Initial Communication
* DONE BUG: sometimes cxref data gets out of sync with actual code (in Emacs)
...
* DONE Make all modules independent on main
** DONE Make refactory.c independent of main
...
* DONE Ensure all ANSI-C, C99 and C11 keywords (and macros for them?) are
...
* DONE __has_include_next() support
** TODO Cover openInclude() with test
* DONE Redefine log levels: TRACE - show what's called & DEBUG - extra information => reorder them
