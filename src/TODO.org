...
** TRY to use LexemBuffer as argument to all putLexXXX() functions
...
****** REQUIRES LexemBuffer as parameter to putLexToken()
...
******* REQUIRES lb->end instead of local variable destination in processDefineDirective()
***** REQUIRES setting next write position in lexemStream in handleCompletionOrSearch()
****** DONE function to set lb->end (setLexemStreamEnd())
*** DONE Use LexemBuffer as argument to putLexLine()
*** TRY Use LexemBuffer as argument to putLexPositionFields()
**** REQUIRES LexemBuffer instead of local destination in processDefineDirective()
**** REQUIRES LexemBuffer instead of local variables in collate()
**** REQUIRES LexemBuffer instead of local variables in createMacroBody()
...
*** TRY Use LexemBuffer as argument to putLexShort()
**** REQUIRES LexemBuffer as argument to putLexToken()
*** TRY Use LexemBuffer as argument to putLexInt()
**** REQUIRES lb->end instead of local variable destination in processDefineDirective()
*** TRY Use LexemBuffer as argument to putLexCompacted()
**** REQUIRES LexemBuffer as argument to putLexPositionFields()
...
* DOING Turn lexembuffer functions into methods on the buffer
  ...
  When do we point to something not a lexembuffer?* DOING Figure out, and if possible, clean up, how 'refactory' calls 'server'
...
* TODO BUG: Progress now (b7c1d) shows fewer of the numbers
