...
* DOING Project local .c-xrefrc with relative file paths so it can be checked in to a repo
...
* TODO Abstract refTab
** TODO Implement table functions as per other tables (getReferenceItem
* TODO Convert fillBits() with explicit setting of "properties" like access, storage etc.
...
* TODO ROADMAP
** DOING LSP-like protocol (possibly also fulfill LSP functions)
