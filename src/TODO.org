...
* DOING Rename vApplClass - but to what? what is it?
* TODO Always pass Position by-value
...
* TODO BUG: sometimes cxref data gets out of sync with actual code (in Emacs)
  E.g. when removing a line in a enum list, enums get extra references where they "used to be".
...
** TODO FEATURE: Rename "module" in C
   Prompt for new "module name", rename both .c & .h if they exist, and change all includes, bonus: rename also _tests.c
...
** TODO Test case for extract variable
