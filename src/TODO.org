...
* TODO Selecting automatic project selection after having set a specific does not seem to work
* TODO Use -trace and -debug to change level in log file only
* TODO Cleanup generateOnlineCxref() which only generates a GOTO
* TODO Cleanup log/messages - now there are a lot of special cases
  We really want command line options to govern what the user/client sees on stdout/stderr(?)
  But if -log is used the log file should always contain everything
* TODO Select extra defines as necessary depending on compiler discovered as a sideeffect of discoverStandardIncludes()
* TODO Include of <file> does not follow standard so Cgreen's 'internal/...' is not found
...
* TODO Add yacc construct for field in yystype ('$<field>$')
