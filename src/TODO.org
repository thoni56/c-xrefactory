...
***** DOING semact.c requires FILL_symbolBits which are not strFill'ed
if symbol.h is not in proto.h. We need to create fillSymbolBits()
***** TODO Replace 0 with ACC_DEFAULT in calls to fillSymbolBits()
...
* TODO Make fillSymbol() and newSymbol() set default symbolBits
