...
** DONE Lookup the closest .c-xrefrc and potentially use it directory path as project name
...
** Basic extract variable
** TRY Place the variable declaration in the closest block (need to figure out how ProgramGraphs work...)
** TRY Figure out variable type (hard, none of the other extracts seems to do this...)
* REQUIRES to make LexemBuffer interface just take LexemBuffer
** DONE Unify LexInput with LexemBuffer so we can get rid of put/get LexemAt() functions
*** to use LexemBuffer as argument to all putLexXXX() functions except in yylex.c
**** to add a LexemBuffer as argument to putLexToken()
***** LexemBuffer as argument to putLexLines()
***** LexemBuffer as argument to processIdentifier()
***** LexemBuffer as argument to putEmptyCompletionId()
***** LexemBuffer instead of body in processDefineDirective() - yylex.c
****** Use putLexXXXWithPointer() for now
***** LexemBuffer instead of buf/bcc/tbcc in expandMacroArgument()
****** Use putLexXXXWithPointer() for now
***** LexemBuffer instead of buf/bcc in createMacroBody()
****** Use putLexXXXWithPointer() for now
***** backpatching of lexems
****** backpatch lexem token in handleCppToken()
******* functions to backpatch lexem token
******* LexemBuffer as parameter to putLexToken()
******** lb->end instead of writePositionP in handleCppToken()
******** lb->end instead of extra parameter in processIdentifier()
******** lb->end instead of extra parameter in getLexemFromLexer()
******** lb->end instead of local variable destination in processDefineDirective()
********* Use putLexTokenWithPointer() instead
****** setting next write position in lexemStream in handleCompletionOrSearch()
******* function to set lb->end (setLexemStreamEnd())
**** Use LexemBuffer as argument to putLexLine()
**** Use LexemBuffer as argument to putLexPositionFields()
***** LexemBuffer instead of local destination in processDefineDirective()
****** Use putLexXXXWithPointer() for now
***** LexemBuffer instead of local variables in collate()
****** Use putLexXXXWithPointer() for now
***** LexemBuffer instead of local variables in createMacroBody()
****** Use putLexXXXWithPointer() for now
**** Use LexemBuffer as argument to putLexPosition()
***** Use lb->end instead of writePositionP in handleCompletionOrSearch()
**** Use LexemBuffer as argument to putLexChar()
**** Use LexemBuffer as argument to putLexInt()
***** lb->end instead of local variable destination in processDefineDirective()
****** Use putLexXXXWithPointer() for now
** Move put...() for complete Lexems (including position, string, ...) to lexembuffer.c
** REQUIRES to use LexemBuffer as argument to all getLexXXX() functions
*** REQUIRES to use LexemBuffer in getLexInt()
**** DOING getExtraLexemInformationFor() to send LexemBuffer instead of pointer
***** DOING currentInput to contain LexemBuffer instead of currentLexemP
*** REQUIRES to use LexemBuffer in getLexToken()
**** DOING getExtraLexemInformationFor() to send LexemBuffer instead of pointer
**** DOING getLexem() and getLexemSavePrevious() to use LexemBuffer instead of pointer
**** DOING collate() to work with LexemBuffer instead of pointers
**** DOING macroArgumentsToString() to work with LexemBuffer instead of LexInput's pointers
*** REQUIRES to use LexemBuffer in getLexPosition()
** REQUIRES to replace putLexXXXWithPointer() in yylex.c
...
* TRY Add yacc parser construct for field in yystype ('$<field>$' and '$<field>3')
  ...
  have it.
* TODO Remove explicit support for Xemacs in elisp
...
** TODO Test case for corruptionError() and compressionError() (jar/zip/class)
...
This is probably not possible due to the level of integration between
the editor adaptor and the c-xref proper. Also LSP does not have a
protocol for many of the refactorings which are core to c-xref.
* TODO BUGS:
