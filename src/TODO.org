...
* TODO Extract PPC-functions from misc.c
...
Initial work done in that we are calling log() but we need to move the
output from dumpOut to some logFile.
...
* TODO Modules. Some things might really belong in separate modules, at
