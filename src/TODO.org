...
** DOING Remove all FILL_xxx macros by using compound initializers or functions:
*** DOING Remove FILL_symbol()
***** DOING Remove #include "symbol" from proto.h
...
* DOING Remove FILLF_fileItem() by replacing it with fillFileItem()
** TODO Make s_noneFileIndex into a constant/define
*** TODO Make TabAdd() take int rather than *int
**** DOING Make IsMember() return position rather than out argument
**** DOING Make addFileTabItem return the position rather than out arg
***** DOING Cover jarFileParse() with test
