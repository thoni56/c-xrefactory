...
* DOING Build a protocol spy that can be put between the editor and the
...
* DOING Refactor out the hashtables and lists to separate modules.
* DOING Start using log.c functions to log to a separate log file
* TODO Move XX_ALLOCC and friends to "memory" module
* TODO Ensure only externally used functions are visible in the modules
header file. And that only those are "non-static" in the C file.
* TODO Fix byacc skeleton to "goto <non-used label" by remove ifdef around "lint"
* TODO Remove bootstrap step (strfill and local definitions)
** TODO Remove all FILL_xxx macros by replacing actually used ones with compound initializers or functions:
...
control logging. A note to self is that setting up the temporary files
are kind of tricky, partly because  of the multi-pass character of the
c-xref (that I don't really get at this point).
...
* TODO Unit tests. There are now some simple ones. We need to get some working as
...
* TODO Make yacc parsing allowed the default. Change Makefile.common so
...
* TODO Move all function prototypes from proto.h to <module>.h
