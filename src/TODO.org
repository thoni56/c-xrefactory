...
** TODO Remove enumTxt generation
This is easy to do but we want to retain the feature that some enums
have there values as strings, and be sure that if we change the enum,
those will change too. Is it possible to do as a check "after the
fact"? Instead of generating them as a pre-requisite, generate and
...
** TODO Make commons.c independent on main
