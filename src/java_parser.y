            ...
            if (regularPass()) {
                ...
                if (inSecondJslPass()) {
                    ...
                    if ($2.data == NULL) {	/* anonymous package */
                        ...
                        javaMapOverDirectoryFiles2($2.data,jslAddMapedImportTypeName,
                                                NULL,$2.data,NULL);
                        // why this is there, it makes problem when moving a class
                        // it stays in fileTab and there is a clash!
                        // [2/8/2003] Maybe I should put it out
    ...
    |	EXTENDS ExtendClassOrInterfaceType			{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    ...
                    assert($2.data->u.typeModifier->type == TypeStruct);
    ...
    :   InterfaceType							{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    ...
                    assert($1.data->u.typeModifier->type == TypeStruct);
    ...
    |	InterfaceTypeList ',' InterfaceType		{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    ...
                    assert($3.data->u.typeModifier->type == TypeStruct);
...
VariableDeclaratorId
    :   Identifier							{
            ...
            if (inSecondJslPass()) {
                ...
                $$.data = cfAlloc(Symbol);
                *$$.data = makeSymbol(name, name, $1.data->position);
            }
        }
    |	VariableDeclaratorId '[' ']'		{
            if (regularPass()) {
        ...
        '(' FormalParameterList_opt ')'
            {
                if (regularPass()) {
                    if (! SyntaxPassOnly()) {
                        $$.data = $<symbol>2;
                        assert($$.data && $$.data->u.typeModifier && $$.data->u.typeModifier->type == TypeFunction);
    ...
    :   Primary '.' New Name _erfs_
            {
                if (ComputingPossibleParameterCompletion()) {
                    ...
                    if ($1.data.typeModifier->type == TypeStruct) {
        ...
        '(' ArgumentList_opt ')'				{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    ...
                    $$.data.idList = $4.data;
                    if ($$.data.typeModifier->type != TypeError) {
...
ArrayAccess
    :   Name '[' Expression ']'							{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    ...
                    if (tt->type==TypeArray) $$.data.typeModifier = tt->next;
    ...
    |	PrimaryNoNewArray '[' Expression ']'		{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    if ($1.data.typeModifier->type==TypeArray) $$.data.typeModifier = $1.data.typeModifier->next;
                    else $$.data.typeModifier = &errorModifier;
                    assert($$.data.typeModifier);
                    $$.data.reference = NULL;
                } else {
...
UnaryExpressionNotPlusMinus
    ...
    |	'!' UnaryExpression		{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    if ($2.data.typeModifier->type == TypeBoolean) $$.data.typeModifier = $2.data.typeModifier;
                    else $$.data.typeModifier = &errorModifier;
                    $$.data.reference = NULL;
                } else {
                    $$.data.position = NULL_POS;
    ...
    :   '(' ArrayType ')' UnaryExpression					{
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    ...
                    $$.data.reference = NULL;
                    assert($$.data.typeModifier->type == TypeArray);
                } else {
                    $$.data.position = NULL_POS;
                    PropagateBoundaries($$, $1, $4);
...
Assignment
    :   LeftHandSide {
            if (regularPass()) {
                if (! SyntaxPassOnly()) {
                    if ($1.data.typeModifier!=NULL && $1.data.typeModifier->type == TypeStruct) {
