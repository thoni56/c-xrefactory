...
Reference *addNewCxReference(Symbol *symbol, Position *position, Usage usage,
    ...
    if (!isMemberInReferenceTable(&referenceItem, &index, &foundMember)) {
        ...
        fillReferenceItem(r, linkName, cxFileHashNumber(linkName), vApplCl, vFunCl, symbol->type,
...
SymbolsMenu *olCreateSpecialMenuItem(char *fieldName, int cfi, Storage storage){
    ...
    fillReferenceItem(&ss, fieldName, cxFileHashNumber(fieldName),
...
static void olPushAllReferencesInBetweenMapFun(ReferenceItem *ri, void *voidDataP) {
    Reference            *rr, *defRef;
    Position              defpos;
    OlcxReferences       *rstack;
    SymbolsMenu          *mm;
    ...
    for (rr=ri->references; rr!=NULL; rr=rr->next) {
        ...
        if (IS_PUSH_ALL_METHODS_VALID_REFERENCE(rr, pushAllData)) {
            ...
            log_trace("adding symbol %s", ri->linkName);
