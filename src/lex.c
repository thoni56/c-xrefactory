...
#define HandleCppToken(ch, cb, dd, cb_next, cb_end, cb_fileNumber, cb_lineNumber, cb_lineBegin, cb_columnOffset) { \
        ...
        if (strcmp(tt,"ifdef") == 0) {                                  \
            ...
            if (ch == '\"' || ch == '<') {                              \
                ...
                scol = columnPosition(cb, cb_lineBegin, cb_columnOffset); \
...
#define CommentaryBegRef(cb, cb_next, cb_fileNumber, cb_lineNumber, cb_lineBegin, cb_columnOffset) { \
        if (s_opt.taskRegime==RegimeHtmlGenerate && s_opt.htmlNoColors==0) { \
            ...
            lcoll = columnPosition(cb, cb_lineBegin, cb_columnOffset) - 1; \
...
#define NOTE_NEW_LEXEM_POSITION(cb, cb_end, lb, cb_fileNumber, cb_lineNumber, cb_lineBegin, cb_columnOffset){ \
