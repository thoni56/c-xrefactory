...
static void editorApplyUndos(EditorUndo *undos, EditorUndo *until, EditorUndo **undoundo, int gen) {
    ...
    while (uu != until && uu != NULL) {
        ...
        case UNDO_REPLACE_STRING:
            ...
            editorReplaceString(uu->buffer, uu->u.replace.offset, uu->u.replace.size, uu->u.replace.str,
                                uu->u.replace.strlen, undoundo);
...
static int getParameterPosition(EditorMarker *pos, char *fname, int argn) {
    ...
    res = RETURN_OK;
    if (s_paramBeginPosition.file == noFileIndex || s_paramEndPosition.file == noFileIndex ||
        s_paramBeginPosition.file == -1 || s_paramEndPosition.file == -1) {
        ppcGotoMarker(pos);
...
static bool addImport(EditorMarker *point, EditorRegionList **regions, char *iname, int line, int vApplCl,
    ...
    if (ld1 == NULL) {
        ...
        if (isTheImportUsed(mm, line, icoll)) {
            ...
            editorFreeMarkersAndRegionList(wholeBuffer);
            wholeBuffer = NULL;
...
#endif
        editorFreeMarkersAndMarkerList(totalDiff);
        totalDiff = NULL;
        askForReallyContinueConfirmation();
    }
}
...
static void refactorVirtualToStatic(EditorMarker *point) {
        ...
        if (mm->selected && mm->visible) {
            ...
            for (EditorMarkerList *ll = mm->markers; ll != NULL; ll = ll->next) {
                ...
                if (IS_DEFINITION_OR_DECL_USAGE(ll->usage.kind)) {
                    ...
                    nparamdefpos = editorCreateNewMarker(pp->buffer, pp->offset + strlen(pardecl) -
                                                                         strlen(refactoringOptions.refpar1));
                } else {
    ...
    editorFreeMarkersAndMarkerList(allrefs);
    allrefs = NULL;
...
static bool isMethodPartRedundantWrtPullUpPushDown(EditorMarker *m1, EditorMarker *m2) {
    ...
    regions = NULL;
    buf     = m1->buffer;
    regions = newEditorRegionList(editorCrMarkerForBufferBegin(buf), editorDuplicateMarker(m1), regions);
    regions = newEditorRegionList(editorDuplicateMarker(m2), editorCrMarkerForBufferEnd(buf), regions);
    ...
    while (mm1 != NULL) {
        ...
        for (EditorMarkerList *rr1 = mm1->markers; rr1 != NULL; rr1 = rr1->next) {
            ...
            if (mm2 == NULL) {
                ...
                *rr = (EditorMarkerList){
                    .marker = editorDuplicateMarker(rr1->marker), .usage = rr1->usage, .next = diff};
                diff = rr;
            }
        }
    cont:
...
static void pullUpPushDownCheckCorrespondance(SymbolsMenu *menu1, SymbolsMenu *menu2, ReferencesItem *theMethod) {
    ...
    if (diff != NULL) {
        ...
        editorFreeMarkersAndMarkerList(diff);
        diff = NULL;
        askForReallyContinueConfirmation();
    }
}
