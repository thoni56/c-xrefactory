...
extern S_reference *olcxAddReferenceNoUsageCheck(S_reference **rlist, S_reference *ref, int bestMatchFlag);
extern S_reference *olcxAddReference(S_reference **rlist,S_reference *ref,int bestMatchFlag);
...
extern S_reference * addCxReferenceNew(S_symbol *p, S_position *pos, S_usageBits *ub, int vFunCl, int vApplCl);
extern S_reference * addCxReference(S_symbol *p, S_position *pos, int usage, int vFunClass,int vApplClass);
extern S_reference *addSpecialFieldReference(char *name, int storage, int fnum, S_position *p, int usage);
...
extern void addTrivialCxReference (char *name, int symType, int storage, S_position *pos, int usage);
extern void olcxAddReferences(S_reference *list, S_reference **dlist, int fnum, int bestMatchFlag);
extern void olSetCallerPosition(S_position *pos);
extern S_olCompletion * olCompletionListPrepend(char *name, char *fullText, char *vclass, int jindent, S_symbol *s, S_symbolRefItem *ri, S_reference *dfpos, int symType, int vFunClass, S_olcxReferences *stack);
extern S_olSymbolsMenu *olCreateNewMenuItem(
...
extern S_olSymbolsMenu *olAddBrowsedSymbol(
...
extern int itIsSymbolToPushOlRefences(S_symbolRefItem *p, S_olcxReferences *rstack, S_olSymbolsMenu **rss, int checkSelFlag);
...
extern void olcxReferencesDiff(S_reference **anr1, S_reference **aor2,S_reference **diff);
...
extern void changeMethodReferencesUsages(char *linkName, int category, int fnum, S_symbol *cclass);
...
extern int symbolsCorrespondWrtMoving(S_olSymbolsMenu *osym,S_olSymbolsMenu *nsym,int command);
...
extern S_symbol *getMoveTargetClass();
extern int javaGetSuperClassNumFromClassNum(int cn);
extern int javaIsSuperClass(int superclas, int clas);
extern void pushLocalUnusedSymbolsAction();
extern void mainAnswerEditAction();
extern void freeOldestOlcx();
...
extern int smartReadFileTabFile();
